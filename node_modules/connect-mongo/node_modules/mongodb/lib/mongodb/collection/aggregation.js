...
var group = function group(keys, condition, initial, reduce, finalize, command, options, callback) {
  ...
  if(command) {
    ...
    var readPreference = shared._getReadConcern(this, options);
    // Execute the command
    this.db._executeQueryCommand(cmd, {read: readPreference}, utils.handleSingleCommandResultReturn(null, null, function(err, result) {
...
var aggregate = function(pipeline, options, callback) {
  ...
  options = opts.readPreference || opts.explain || opts.cursor || opts.out || opts.allowDiskUsage ? args.pop() : {}
