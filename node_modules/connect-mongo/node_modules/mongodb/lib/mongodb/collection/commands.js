...
var count = function count(query, options, callback) {
  ...
  // Execute the command
  this.db._executeQueryCommand(DbCommand.createDbSlaveOkCommand(this.db, commandObject), {read: readPreference}, utils.handleSingleCommandResultReturn(null, null, function(err, result) {
...
var rename = function rename(newName, options, callback) {
  ...
  self.db._executeQueryCommand(DbCommand.createRenameCollectionCommand(self.db, self.collectionName, newName, options), utils.handleSingleCommandResultReturn(true, false, function(err, result) {
      if(err) return callback(err, null)
      try {
