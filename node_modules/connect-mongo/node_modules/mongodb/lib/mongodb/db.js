...
function Db(databaseName, serverConfig, options) {
  ...
  // Set up logger
  this.logger = this.options.logger != null && (typeof this.options.logger.debug == 'function') && (typeof this.options.logger.error == 'function') && (typeof this.options.logger.log == 'function') ? this.options.logger : {error: function(message, object) {
  ...
  // but the db one was
  if(this.serverConfig.options.readPreference == null && this.readPreference != null) {
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  self.command(command, commandOptions, function(err, result) {
    ...
    callback(!result.ok ? utils.toError("Failed to add user " + username) : null, result.ok ? [
        {user: username, pwd: ''}
      ] : null);
  })
}
...
Db.prototype.createCollection = function(collectionName, options, callback) {
  ...
  this.collectionNames(collectionName, function(err, collections) {
    ...
    self._executeQueryCommand(DbCommand.createCreateCollectionCommand(self, collectionName, options), {read: false, safe: safe}, utils.handleSingleCommandResultReturn(null, null, function(err, result) {
        if(err) return callback(err, null);
...
Db.prototype.command = function(selector, options, callback) {
  ...
  if(readPreference != false && ignoreCommandFilter == false) {
    if(selector['group'] || selector['aggregate'] || selector['collStats'] || selector['dbStats'] || selector['count'] || selector['distinct'] || selector['geoNear'] || selector['geoSearch'] || selector['geoWalk'] || selector['text'] || (selector['mapreduce'] && (selector.out == 'inline' || selector.out.inline))) {
...
Db.prototype.dropCollection = function(collectionName, callback) {
  ...
  // Drop the collection
  this._executeQueryCommand(DbCommand.createDropCollectionCommand(this, collectionName), utils.handleSingleCommandResultReturn(true, false, callback));
};
  ...
  this.indexInformation(collectionName, function(err, collectionInfo) {
    ...
    if(!collectionInfo[index_name]) {
      ...
      if(_hasWriteConcern(errorOptions) && typeof callback == 'function') {
        ...
        if(typeof callback === 'function' && commandOptions.w < 1 && !commandOptions.fsync && !commandOptions.journal) {
          commandOptions.w = 1;
        }
...
Db.prototype.cursorInfo = function(options, callback) {
  ...
  this._executeQueryCommand(DbCommand.createDbSlaveOkCommand(this, {'cursorInfo': 1}), options, utils.handleSingleCommandResultReturn(null, null, callback));
};
...
Db.prototype.dropIndex = function(collectionName, indexName, callback) {
  this._executeQueryCommand(DbCommand.createDropIndexCommand(this, collectionName, indexName), utils.handleSingleCommandResultReturn(null, null, callback));
};
...
Db.prototype.reIndex = function(collectionName, callback) {
  this._executeQueryCommand(DbCommand.createReIndexCommand(this, collectionName), utils.handleSingleCommandResultReturn(true, false, callback));
};
...
Db.prototype.dropDatabase = function(callback) {
  this._executeQueryCommand(DbCommand.createDropDatabaseCommand(this), utils.handleSingleCommandResultReturn(true, false, callback));
}
    ...
    connection.write(db_command, function(err) {
      if(err != null) {
        ...
        if(Array.isArray(db_command))
          self.serverConfig._callHandler(db_command[0].getRequestId(), null, err); else
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  if(!connection && !config.canRead(read) && !config.canWrite() && config.isAutoReconnect()) {
    if(read == ReadPreference.PRIMARY || read == ReadPreference.PRIMARY_PREFERRED || (read != null && typeof read == 'object' && read.mode) || read == null) {

      // Save the command
      self.serverConfig._commandsStore.read_from_writer({   type: 'query', db_command: db_command, options: options, callback: callback, db: self, executeQueryCommand: __executeQueryCommand, executeInsertCommand: __executeInsertCommand
        });
    } else {
      self.serverConfig._commandsStore.read({   type: 'query', db_command: db_command, options: options, callback: callback, db: self, executeQueryCommand: __executeQueryCommand, executeInsertCommand: __executeInsertCommand
        });
    }
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  if(!connection && !config.canWrite() && config.isAutoReconnect()) {
    self.serverConfig._commandsStore.write({   type: 'insert', 'db_command': db_command, 'options': options, 'callback': callback, db: self, executeQueryCommand: __executeQueryCommand, executeInsertCommand: __executeInsertCommand
      });
...
Db.connect = function(url, options, callback) {
  ...
  if(url.indexOf("safe") == -1 && url.indexOf("w") == -1 && url.indexOf("journal") == -1 && url.indexOf("j") == -1 && url.indexOf("fsync") == -1) options.w = 0;
