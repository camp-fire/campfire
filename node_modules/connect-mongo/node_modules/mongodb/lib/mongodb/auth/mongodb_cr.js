  ...
  db._executeQueryCommand(DbCommand.createGetNonceCommand(db), options, function(err, result, connection) {
    ...
    if(err == null) {
      ...
      db._executeQueryCommand(DbCommand.createAuthenticationCommand(db, username, password, nonce, authdb), {connection: connection}, function(err, result) {
        ...
        if(err) {
          errorObject = err;
        } else if(result && Array.isArray(result.documents) && result.documents.length > 0 && (result.documents[0].err != null || result.documents[0].errmsg != null)) {
        ...
        if(numberOfConnections <= 0 && typeof callback == 'function') {
          ...
          if(errorObject == null && result && Array.isArray(result.documents) && result.documents.length > 0 && result.documents[0].ok == 1) {            // We authenticated correctly save the credentials
